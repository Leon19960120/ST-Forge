...
> **Windows 注意**：确保 ARM GCC 和 OpenOCD 的 `bin` 目录已加入系统 PATH，Cortex-Debug 才能找到 `arm-none-eabi-gdb` 和 `openocd`。如果 PATH 方式不生效，可在 VSCode 的 `settings.json` 中手动配置 `cortex-debug.armToolchainPath` 和 `cortex-debug.openOCDPath`。详见 [01-环境配置.md](./01-环境配置.md)。
...
4. 详见 [01-环境配置.md](./01-环境配置.md) 中的 ST-Link 驱动安装章节

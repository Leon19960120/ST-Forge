...
确保 ARM GCC 工具链和 OpenOCD 的 `bin` 目录已加入系统 PATH（参见 [01-环境配置](./01-环境配置.md)）。Cortex-Debug 会自动在 PATH 中查找 `arm-none-eabi-gdb` 和 `openocd`。
...
**上一章**：[编译与烧录](./04-编译与烧录.md)

**下一章**：[STM32基础概念](./06-STM32基础概念.md)

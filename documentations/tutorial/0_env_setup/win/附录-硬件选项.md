...
| 驱动兼容性 | 可能需要安装特定驱动 | 使用 Zadig 替换驱动为 WinUSB（详见下方说明） |

**Windows 上克隆版 ST-Link 的驱动处理：**

部分 ST-Link 克隆版使用不同的 USB VID/PID，Windows 无法通过官方 STSW-LINK009 驱动识别。需要使用 Zadig 工具：

1. 下载 [Zadig](https://zadig.akeo.ie/)（Windows USB 驱动替换工具）
...
步骤 4：电脑端排查
- 在设备管理器中查看是否有未知设备
- 检查 ST-Link 驱动是否正确安装（应显示为 "STMicroelectronics STLink dongle"）
- 如果显示为未知设备，安装 STSW-LINK009 驱动或使用 Zadig 替换驱动
- 尝试其他电脑进行交叉验证
```
...
4. 软件设置
   - 确认选择了正确的芯片型号
   - 检查调试接口设置（SWD/JTAG）
   - 在 Windows 上，确认设备管理器中 ST-Link 驱动已正确安装
```
